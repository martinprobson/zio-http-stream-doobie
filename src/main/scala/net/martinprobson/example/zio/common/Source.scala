...
case class MemorySource(private val size: Int) extends Source:
  val layer: ZLayer[Any, Throwable, MemorySource] = ZLayer.succeed(MemorySource(size))
...
//object MemorySource:
//  val layer: ZLayer[Any, Exception, MemorySource] = ZLayer.succeed(MemorySource(10))
